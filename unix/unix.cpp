...
/*
	if(which1 == 0) {
		...
		if (keyssnes[sfc_key[UP_1]] == SDL_PRESSED || joy_axes[which1][joyaxis_UD_1] == UP)		val |= SNES_UP_MASK;
		if (keyssnes[sfc_key[DOWN_1]] == SDL_PRESSED || joy_axes[which1][joyaxis_UD_1] == DOWN)	val |= SNES_DOWN_MASK;
		if (keyssnes[sfc_key[LEFT_1]] == SDL_PRESSED || joy_axes[which1][joyaxis_LR_1] == LEFT)	val |= SNES_LEFT_MASK;
		if (keyssnes[sfc_key[RIGHT_1]] == SDL_PRESSED || joy_axes[which1][joyaxis_LR_1] == RIGHT)	val |= SNES_RIGHT_MASK;
	} else {
